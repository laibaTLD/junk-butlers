...
function GalleryCardImage({
  ...
  const fallback = getGalleryFallbackImage(fallbackIndex);
...
function buildDefaultGalleryImages(): Image[] {
  const now = new Date().toISOString();
  return GALLERY_FALLBACK_IMAGES.map((imageUrl, index) => ({
    id: `fallback-${index + 1}`,
    slotName: `gallery-image-${index + 1}`,
    title: `Gallery ${index + 1}`,
    altText: "Junk Butlers team at work",
    imageUrl,
    category: "gallery",
...
export default function GallerySection({
  ...
  const resolvedGallery = images?.length ? getGalleryImages(images) : [];
  const galleryImages =
    resolvedGallery.length > 0
      ? resolvedGallery
      : buildDefaultGalleryImages();

  const { ref: gridRef, visibleItems } = useStaggeredAnimation(
    Math.min(galleryImages.length, 8),
    150
  );
